(async () => {
	...
	function canvasUpdate(time) {
		...
		imageParams.offsetX = Math.max(
			Math.min(0, imageParams.offsetX),
			canvas.width - image.width * Math.abs(imageParams.scale)
		);
		imageParams.offsetY = Math.max(
			Math.min(0, imageParams.offsetY),
			canvas.height - image.height * Math.abs(imageParams.scale)
		);
